...
const Posts = (props) => {
    ...
    const onChangeInput = (id, event) => {
        ...
        props.editPost(inputRef.current?.value, id, props.index)
        toggleState();
        ...
        isEdit ? (
            <div>
                ...
                <ul className="card  posts">
                    ...
                    <li className="card-body post">{props.post.post}</li>
                    <hr />
                    <InnerImageZoom src={'http://localhost:3000/' + props.post.avatar} />
                    <ul className="card-footer text-center button-post">
                        <li style={color} id="like" onClick={() => props.LikePost(props.post._id) && plusLike()} >إعجاب </li>
                        <li id="delete" style={style} onClick={() => props.deletPost(props.index, props.post._id)}>حذف المنشور</li>
                        <li style={style} onClick={() => toggleState()}>تعديل المنشور</li>
            ...
            : (
                <form onSubmit={(event) => onChangeInput(props.post._id, event)}>
                    <ul className="card  posts">
                        ...
                        <hr />
                        <img src={'http://localhost:3000/' + props.post.avatar} />
